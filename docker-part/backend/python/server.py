...
# print(">>> KEYCLOAK_CLIENT_SECRET =", os.getenv("KEYCLOAK_CLIENT_SECRET"))
# print(">>> KEYCLOAK_ADMIN_SECRET =", os.getenv("KEYCLOAK_ADMIN_SECRET"))
...
# #     admin_client_secret=os.getenv("KEYCLOAK_ADMIN_SECRET"),
# #     realm=os.getenv("KEYCLOAK_REALM"),
# #     callback_uri="http://localhost:8000/callback",  # Może być dowolne, nie używasz logowania frontendowego
# # )

# keycloak = FastAPIKeycloak(
#     server_url="http://host.docker.internal:8080/",
#     client_id="backend",
#     client_secret="15HJeFLR2c0FHuhoODncX3llBAozaGxv",
#     admin_client_secret="pCV6vQ5zGeMZUHMqjclw7JIVJiRgwPdR",
#     realm="task-realm",
#     callback_uri="http://localhost:8000/callback",  # Może być dowolne, nie używasz logowania frontendowego
# )
...
#         password=open(os.getenv("PGPASSWORD_FILE")).read().strip(),
...
# async def create_task_by_admin(
#     task: TaskInput,
#     user=Depends(keycloak.get_current_user(required_roles=["admin"]))
# ):
#     conn = await get_db()
#     try:
#         task_record = await conn.fetchrow(
...
# @app.get("/api/employees")
# async def get_employees(user=Depends(keycloak.get_current_user(required_roles=["admin"]))):
#     conn = await get_db()
#     try:
#         employees = await conn.fetch("SELECT username AS email FROM user_entity WHERE email IS NOT NULL")
#         return {"employees": [dict(e) for e in employees]}
#     finally:
#         await conn.close()
